// =============================================================
// Taller App · Edge Function: crear cuenta de mecánico
//
// Se invoca desde la pantalla "Gestión de personal" (solo visible para un
// encargado) — supabase.functions.invoke('crear-cuenta-mecanico', {body:
// {nombre, email, password}}). Crea una cuenta de Supabase Auth nueva y le
// asigna rol 'mecanico' en la tabla `perfiles`.
//
...
//
// Seguridad: antes de crear nada, esta función comprueba que quien la
// llama es realmente un 'encargado' (con su propio token de sesión, sin
// privilegios de admin) — si no lo es, o si no hay token, se rechaza. Solo
// DESPUÉS de esa comprobación se usa la clave service_role para crear la
// cuenta. Sin esta comprobación, cualquier persona con el enlace de la
// función podría crear cuentas arbitrarias.
//
...
//
// Si esta función no está desplegada, el botón "Crear mecánico" de la app
// falla con un aviso claro — el resto de la app funciona igual, esto no es
// un requisito para nada más.
// =============================================================
...
serve(async (req) => {
  ...
  try {
    ...
    if (errorPerfil || perfilLlamante?.rol !== 'encargado') {
      return jsonError('Solo un encargado puede crear cuentas de mecánico.', 403);
    }

    const { nombre, email, password } = (await req.json()) as Body;
    ...
    // arriba que quien llama es un encargado.
    ...
    const { data: nuevoUsuario, error: errorCrear } = await clienteAdmin.auth.admin.createUser({
      email: email.trim(),
      password,
      email_confirm: true, // no hace falta que el mecánico confirme el email
    ...
    // rol por defecto 'cliente' — se corrige aquí a 'mecanico'. El cliente
    // admin (service_role) bypasea RLS, así que esto funciona siempre.
    const { error: errorActualizarPerfil } = await clienteAdmin
      .from('perfiles')
      .update({ rol: 'mecanico', nombre: nombre.trim(), email: email.trim() })
